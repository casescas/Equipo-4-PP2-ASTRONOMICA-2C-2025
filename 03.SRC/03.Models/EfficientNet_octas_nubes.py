...
train_dataset = OctaDataset(train_paths, train_labels, img_width, img_height, augment=True, minority_classes=minority_classes, oversample_factor=3)
val_dataset = OctaDataset(val_paths, val_labels, img_width, img_height, augment=False)

train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)

# Modelo EfficientNet con Fine-Tuning completo
...
criterion = FocalLoss()
optimizer = optim.Adam(model.parameters(), lr=1e-5, weight_decay=1e-4)  # LR bajo para fine-tuning completo
